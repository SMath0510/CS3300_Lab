...
function testRA {
    ...
    if [[ $nc == 0 ]] && [[ $j2m == 1 ]]; then
        ...
        if [[ -f tests/input/IR/$name.microIR ]]; then
            java -jar tests/M_R.jar < tests/input/IR/$name.microIR 1>tests/input/$dir/$name.miniRA 2>/dev/null
        fi
    fi
    ...
    then
        ...
        if [[ $debug == 0 ]]
        then
            java Main < tests/input/$dir/$name.miniRA 1>tests/output/MIPS/mine/$name.s 2>/dev/null
        else
            java Main < tests/input/$dir/$name.miniRA 1>tests/output/MIPS/mine/$name.s
        fi
    fi
    ...
    then
        ...
        cat tests/output/exec/mine/$name | grep -v "Loaded" | grep -v "SPIM" | grep -v "Copyright" | grep -v "All" | grep -v "See"> tmp
